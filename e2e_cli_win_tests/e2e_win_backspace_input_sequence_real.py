...
class TestWinBackspaceInputSequenceReal(unittest.TestCase):
    def test_backspace_deletes_single_character_not_last_word(self) -> None:
        ...
        turn1 = f"INIT({token}): Reply with exactly the word ONE."

        # Repro: type a line with a space, press Backspace, and continue typing.
        # On Windows with VT input, DEL (0x7f) can incorrectly delete the whole last word.
        typed = f"BACKSPACE({token}) abc defX"
        expected = f"BACKSPACE({token}) abc defY"
