...
class TestWinCtrlCDuringResponseReal(unittest.TestCase):
    def test_ctrl_c_during_response_returns_to_prompt_and_next_turn_still_works(self) -> None:
        ...
        turn1 = (
            f"CTRL_C_INFLIGHT({token}): Start replying with at least 15000 characters. "
            "Do not use code blocks. Keep going until interrupted."
        )
        turn2 = f"CTRL_C_AFTER({token}): Reply with exactly the word OK."
