...
class TestE2EToolsListTruncatedLimitRealInjected(unittest.IsolatedAsyncioTestCase):
    async def test_list_truncates_at_default_limit_and_reports_truncated(self) -> None:
        with TemporaryDirectory() as td:
            root = Path(td)
            # Default limit is 100 (see ListTool.limit).
            for i in range(105):
            ...
            r = await openagentic_sdk.run(prompt="Run injected List for truncation test.", options=opts)
            self.assertEqual((r.final_text or "").strip(), "LIST_TRUNC_OK")
            ...
            self.assertEqual(out.get("count"), 100)
            self.assertEqual(out.get("truncated"), True)

            tree = out.get("output")
            self.assertIsInstance(tree, str)
            self.assertIn("f000.txt", tree)
            self.assertIn("f099.txt", tree)
            self.assertNotIn("f104.txt", tree)
