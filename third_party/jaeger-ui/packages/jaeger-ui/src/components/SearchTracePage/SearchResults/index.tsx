  ...
  return (
    <label>
      ...
      <SearchableSelect value={sortBy} onChange={(value: string) => handleSortChange(value)}>
        <Option value={orderBy.MOST_RECENT}>最新优先</Option>
        <Option value={orderBy.LONGEST_FIRST}>最长优先</Option>
        <Option value={orderBy.SHORTEST_FIRST}>最短优先</Option>
        <Option value={orderBy.MOST_SPANS}>Span 数最多</Option>
        <Option value={orderBy.LEAST_SPANS}>Span 数最少</Option>
      </SearchableSelect>
    </label>
  );
}
