# Rust SDK（核心模块 / 不含 CLI）E2E 测试 Checklist

面向：正在把 `openagentic-sdk` 的核心能力移植到 Rust 生态（可用于 server/desktop/wasm 的 SDK 形态）的同学。  
...
- [ ] 支持“确定性调度”：对并发/异步路径，测试里能强制串行或固定顺序。
- [ ] 支持“可插入时钟/随机源”：用 fake clock、固定 seed，避免时间/随机导致 flake。
- [ ] 支持“响应变形器”：同一语义用不同字段顺序/空白/可选字段，验证解析鲁棒性。

> Rust 提示（常用做法）：把 `now()`/`sleep()`/`rng()` 抽象成 trait 注入；如果使用 tokio，可用 `#[tokio::test(start_paused = true)]` + `tokio::time::advance` 让 backoff/timeout 测试不变慢。

### 2.2 Golden Trace（关键事件对齐 / 追溯）

建议产物：
- `events.jsonl`（或 Rust 端等价物）：每行一个 JSON 事件。
...
> Rust 提示：JSON 对象 key 的顺序不应作为断言依据；Golden 对齐建议在“解析后结构”上做 canonicalization（递归排序 key、规范化 `null` vs 缺字段的语义）。
...
- [ ] `TempSessionDir`：所有测试写入临时目录，绝不污染 repo（Rust 可用 `tempfile`）。
- [ ] `InMemorySessionStore`（可选）：用于极少数“只测事件逻辑不测 IO”的用例，但 hard invariants 仍建议走真实文件 IO。
- [ ] `EventAsserts`：统一断言工具（事件序列、关联关系、字段存在性、敏感信息脱敏）。
- [ ] `ToolTestDoubles`：标准工具集合（成功/失败/慢/大输出/非 JSON 输出/副作用工具）。
- [ ] `NetworkStubs`（若有在线 contract）：可替换 HTTP client（reqwest/hyper）或使用本地 stub server。
...
- [ ] **取消/中断**：中途取消会话不会留下半行 JSON；恢复策略明确（例如标记 cancelled/aborted）。
...
- [ ] **资源回收**：会话结束后不遗留后台任务；多次运行无明显泄漏（粗粒度 smoke）。
...
- [ ] **prompt 模式**：有回答器可继续；无交互时不死等（快速失败/返回错误）。
...
- [ ] **拒绝后行为**：deny 后继续对话/返回错误/要求模型改计划等策略明确可测。
...
- [ ] **不重放副作用**：恢复时不重复执行已执行过的副作用工具（除非明确设计要重放）。
- [ ] **多会话隔离**：不同 session id 不串；并发写不会互相覆盖（或明确拒绝并发并报错清晰）。
- [ ] **事件膨胀防护**：长对话下 trace 规模可控（compaction 或上限策略可测）。
- [ ] **跨平台**：Linux/macOS/Windows 文件锁、路径大小写、符号链接、长路径差异下语义一致（或明确限制）。
...
- [ ] **流式协议解析（如有）**：SSE/chunked JSON 半包/粘包、空行、非法 JSON 下行为可预测且不写坏 session。
- [ ] **HTTP 细节**：重定向、代理、TLS、证书、DNS 失败能分类清楚（至少不要吞错）。
- [ ] **请求幂等**：重试不会导致同一请求产生两个 run；run_id/trace 关联清晰。
- [ ] **可观测性**：provider 提供 request_id/response_id 时写入 trace（脱敏后）。

### 3.9 配置与跨平台一致性（Rust 生态常见坑）

- [ ] **配置优先级**：参数 vs env vs 默认优先级固定且可测。
- [ ] **路径规范化**：`..`、绝对/相对路径、符号链接跳转，不能突破 sandbox 根目录（canonical path 校验）。
- [ ] **编码与换行**：LF/CRLF 不影响解析；文本工具输出换行策略一致。
- [ ] **序列化语义**：serde 对缺字段/默认值/`null` 的语义差异必须覆盖（`null` vs 缺字段尤其关键）。
- [ ] **数字语义**：JSON 数字到 `i64`/`f64` 的精度与溢出策略明确。
- [ ] **时区/locale**：时间戳编码/解析不受 locale 影响。
- [ ] **async runtime**：tokio/async-std 等运行时差异不会改变核心语义（至少在你们选择的 runtime 上有覆盖）。
...
- [ ] **路径注入**：符号链接/junction 不能逃逸 sandbox 根目录。
...
- [ ] **共享资源争用**：同 session store 根目录并发写不覆盖（或明确拒绝并发并报错清晰）。
- [ ] **背压与队列**：大量 tool calls 时不会无限堆积内存（有队列上限/拒绝策略）。

### 3.13 兼容性与“对齐 Python 参考实现”（推荐）

- [ ] **同脚本、同工具、同配置**：Rust 端与 Python 端产出的关键事件集合一致（允许噪声字段不同）。
...
### 3.14 Skills / Commands / Project 兼容（如 Rust 端要做功能对齐）
...
## 5. 评审者用 Checklist（你给 Rust 同学 Code Review 时可用）
...
- [ ] 是否有追溯矩阵门禁，防止套件膨胀后失控？
- [ ] tokio 时间暂停/取消路径是否有用例（避免 backoff/timeout 测试变慢或 flake）？
...
- [ ] 有且只有一个 `session_id`。
- [ ] `user_message`/`assistant_message` 字段完整。
- [ ] 每个 `tool_use`：`call_id` 存在且唯一；tool name/args 可解释。
- [ ] 每个 `tool_result`：能关联到 `call_id`；成功/失败状态明确。
- [ ] 不出现任何 streaming delta（`*_delta`）。
- [ ] trace 中不出现 secrets（敏感字段黑名单断言）。
- [ ] 事件顺序满足基本因果（或具备可恢复语义）。

### 6.3 建议的失败消息规范

- [ ] 输出：`session_id`、`run_id`（若有）、`call_id`、事件 `seq`/索引、以及对应事件的最小 JSON 摘要。
- [ ] 对字段缺失/类型错误/未知事件给出明确提示。
...
### 7.2 Chaos / Cancellation（异步取消与资源回收）
...
- [ ] **`Option<T>`/默认值语义**：serde 的缺字段/默认值/`null` 语义差异导致“看似一样但语义不同”。
- [ ] **取消传播**：`select!`/`JoinHandle` 取消后仍继续执行导致副作用；必须用例覆盖。
- [ ] **并发竞态**：`Arc<Mutex<_>>` 的隐藏死锁、`RwLock` 饥饿、跨 await 持锁；至少有并发 smoke。
- [ ] **文件系统差异**：Windows 锁/路径大小写/长路径；session store 必测。
- [ ] **HTTP 客户端差异**：reqwest/hyper 的重试、重定向、代理/TLS 行为；错误分类别混成一个“网络错误”。
