...
- **M3: HTTP Actor Transport Adapter**
  ...
  - Status: doing（本地单元/集成/ruff 已完成 2026-03-29；k3d actor e2e 已补齐，当前 shell 缺 docker，尚未在此处实跑）
...
- REQ-0057-006 → `docs/plan/v57-host-subagent-http-transport-adapter.md` → `tests.test_actor_remote_replay` + `e2e_k3d_tests/e2e_remote_actor_reconnect.py` → local replay unit executed 2026-03-29；k3d replay smoke authored，current shell skipped due missing docker
- REQ-0057-007 → `docs/plan/v57-host-subagent-actor-protocol-foundation.md` + `docs/plan/v57-host-subagent-http-transport-adapter.md` → `tests.test_subagent_task` + `tests.test_remote_task_dispatch` + `tests.test_remote_http_transport` → local Task compatibility + remote abort/child event flow executed 2026-03-29；k3d e2e pending docker-enabled run
- REQ-0057-008 → `docs/plan/v57-host-subagent-supervision-and-recovery.md` + `docs/plan/v57-host-subagent-http-transport-adapter.md` → `tests.test_actor_supervision` + `tests.test_cli_trace_renderer` + `tests.test_actor_http_transport` + `tests.test_remote_task_dispatch` → M2 local supervision + CLI trace 已执行；M3 local HTTP identity/down/send/abort trace surfaces executed 2026-03-29
...
- REQ-0057-009 → 全部 v57 计划 → `tests.test_actor_*` + `e2e_k3d_tests/e2e_remote_actor_*.py` → local actor/http unit+integration executed 2026-03-29；k3d actor basic/reconnect authored，current shell skipped due missing docker
...
- 当前 progress：M1/M2 已完成本地验证；M3 已完成本地实现与回归，并补齐 k3d basic/reconnect e2e；剩余缺口是 docker-enabled 环境下的 M3 实跑与 M4 tracing。
