...
class TestWinCrProgressNoiseTypeaheadBackspaceReal(unittest.TestCase):
    def test_cr_progress_noise_does_not_break_typeahead_backspace(self) -> None:
        ...
        typed = f"Turn2_CR_NOISE ({token}): Reply with exactly the word OK. PREFIX abc defX"
        expected = f"Turn2_CR_NOISE ({token}): Reply with exactly the word OK. PREFIX abc defY"
