...
class TestE2ERuntimeToolNotAllowedReal(unittest.IsolatedAsyncioTestCase):
    async def test_tool_not_allowed_emits_tool_result_without_tool_use(self) -> None:
        with TemporaryDirectory() as td:
            ...
            async def inject_read_then_finish(payload: Mapping[str, Any]) -> HookDecision:
                ...
                if stage == 0:
                    stage = 1
                    return HookDecision(
                        action="inject_read_not_allowed",
            ...
            hooks = HookEngine(after_model_call=[HookMatcher(name="inject-read-not-allowed", tool_name_pattern="*", hook=inject_read_then_finish)])
            opts0 = make_options(root, allowed_tools=["Glob"], hooks=hooks)
            opts = replace(opts0, max_steps=5)

            events: list[object] = []
            async for ev in openagentic_sdk.query(prompt="Try to read ./fixture.txt", options=opts):
                events.append(ev)

            tool_uses = [e for e in events if getattr(e, "type", None) == "tool.use" and getattr(e, "name", None) == "Read"]
            self.assertFalse(tool_uses)
            ...
            self.assertEqual(getattr(denied[-1], "error_type", None), "ToolNotAllowed")
