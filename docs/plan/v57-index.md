...
- REQ-0057-001 → `docs/plan/v57-host-subagent-actor-protocol-foundation.md` → `tests.test_actor_protocol` → planned；not yet executed
- REQ-0057-002 → `docs/plan/v57-host-subagent-actor-protocol-foundation.md` → `tests.test_actor_mailbox_store` + `tests.test_actor_local_transport` → planned；not yet executed
- REQ-0057-003 → `docs/plan/v57-host-subagent-actor-protocol-foundation.md` + `docs/plan/v57-host-subagent-http-transport-adapter.md` → `tests.test_actor_local_transport` + `tests.test_actor_http_transport` → planned；not yet executed
...
- 本轮目标：先把 actor foundation 写硬、测试写硬，再进入执行；不提前碰 UI/CLI bridge 扩 scope。
