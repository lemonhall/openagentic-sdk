...
class TestE2EPermPromptDenyThenAllowWriteRealInjected(unittest.IsolatedAsyncioTestCase):
    async def test_prompt_mode_denies_first_write_allows_second_write(self) -> None:
        ...
        with TemporaryDirectory() as td:
            ...
            hooks = HookEngine(
                after_model_call=[HookMatcher(name="inject-writes", tool_name_pattern="*", hook=inject_two_writes_then_finish)]
            )
            ...
            r = await openagentic_sdk.run(prompt="Run injected deny/allow write flow.", options=opts)
            ...
            self.assertEqual((r.final_text or "").strip(), "DENY_ALLOW_OK")
