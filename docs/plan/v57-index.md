...
- **M3: HTTP Actor Transport Adapter**
  ...
  - Status: doing（当前真实完成度：本地单元/集成/ruff 已完成 2026-03-29；worker 级 k3d replay e2e 已补齐但本 shell 缺 docker；remaining gaps 是 mailbox 粒度 replay、显式 ACK、host `Task` 自动 reconnect smoke）
...
- 当前 progress：M1/M2 已完成本地验证；M3 已完成当前 slice 的本地实现与回归，并补齐 worker 级 k3d basic/reconnect e2e；剩余缺口包括 docker-enabled 环境下的 M3 实跑、mailbox 粒度 replay、显式 ACK、以及 host `Task` 自动 reconnect smoke。
