...
class TestWinTypeaheadBackspaceDuringResponseReal(unittest.TestCase):
    def test_typeahead_with_backspace_during_streaming_is_preserved(self) -> None:
        ...
        turn1 = (
            f"Turn1 ({token}): Reply with a long answer (at least 15000 characters), "
            "but do not use code blocks."
        )

        typed = f"Turn2_EDITED ({token}): Reply with exactly the word OK. PREFIX abc defX"
        expected = f"Turn2_EDITED ({token}): Reply with exactly the word OK. PREFIX abc defY"
        ...
        with temp_project_dir() as td:
            ...
            # Force true TTY + VT input path. Allow colored prompt redraw (\r) to better match real users.
            ...
            try:
                ...
                # While Turn1 is still streaming output, type Turn2 WITHOUT pressing Enter.
                time.sleep(0.12)
                p.send(typed)

                # DEL (0x7f) is what many terminals send for Backspace in VT mode.
                p.send("\x7f")
                p.send("Y")
