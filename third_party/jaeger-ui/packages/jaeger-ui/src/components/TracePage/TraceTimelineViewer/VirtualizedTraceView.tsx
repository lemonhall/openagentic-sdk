...
export class VirtualizedTraceViewImpl extends React.Component<VirtualizedTraceViewProps> {
  ...
  detailHeights = new Map<string, number>();
  ...
  _handleDetailMeasure = (evt: { detail?: { height?: number; spanID?: string } }) => {
    const spanID = evt && evt.detail && evt.detail.spanID;
    const detailHeight = evt && evt.detail && evt.detail.height;
    if (!this.listView || !spanID) {
      this._handleListResize();
      return;
    }
    if (typeof detailHeight === 'number' && detailHeight > 0) {
      this.detailHeights.set(spanID, Math.ceil(detailHeight) + 32);
    }
  ...
  getRowHeight = (index: number) => {
    ...
    const measuredHeight = this.detailHeights.get(span.spanID);
