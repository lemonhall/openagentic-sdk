...
- **M4: OTel + Jaeger Tracing**
  ...
  - Status: doing（当前真实完成度：local tracing facade + host/local/remote instrumentation + `tests.test_actor_tracing` 已完成并通过 2026-03-29；Collector/Jaeger manifests、k3d harness tracing bring-up 与直接 `host -> remote worker` 的 `e2e_remote_actor_trace_smoke.py` 已补齐；当前 shell 仍缺 docker，trace e2e 仅完成加载/skip 检查，未实跑）
...
- REQ-0057-011 → `docs/plan/v57-host-subagent-tracing-jaeger.md` → `tests.test_actor_tracing` + `e2e_k3d_tests/e2e_remote_actor_trace_*.py` → local tracing unit executed 2026-03-29；Collector/Jaeger manifests + trace smoke authored；current shell skipped docker-backed trace e2e
...
- 当前 progress：M4 已补上 local tracing facade、host/local/remote instrumentation、Collector/Jaeger manifests 与 trace smoke；剩余缺口主要是 docker-enabled 环境下的 Jaeger trace e2e 实跑与手工 UI 验证。
