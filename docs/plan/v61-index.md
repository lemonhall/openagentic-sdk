...
- **M3: WSL2 冷启动回归**
  ...
  - Status: done
  - Note:
    - `2026-03-30` 实测 `wsl --shutdown` 后到 real host `/health` 恢复约 `279s`
    - 冷启动后 `http://127.0.0.1:16686/api/services` 已重新出现
...
- REQ-0061-001 → `docs/plan/v61-real-runtime-prebaked-image.md` → `tests.test_k3d_real_runtime_image` → done
- REQ-0061-002 → `docs/plan/v61-real-runtime-prebaked-image.md` → `tests.test_k3d_real_runtime_image` + manifest diff review → done
- REQ-0061-003 → `docs/plan/v61-real-runtime-prebaked-image.md` → `tests.test_apply_v56_real_cluster` + apply/build/preload regression → done
- REQ-0061-004 → `docs/plan/v61-real-runtime-prebaked-image.md` → `wsl --shutdown` + same `k3d_chat.py` recovery path cold-start regression → done
- REQ-0061-005 → `docs/plan/v61-real-runtime-prebaked-image.md` → missing-image failure path test + manual error review → done
- REQ-0061-006 → `docs/plan/v61-real-runtime-prebaked-image.md` → scope review + smoke regression check → done
- REQ-0061-007 → `docs/plan/v61-real-runtime-prebaked-image.md` → real rollout + Jaeger services verification → done
